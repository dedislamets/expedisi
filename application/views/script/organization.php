...
<script type="text/javascript">
	$('#include-sub').on('click', function () {
	    $('#jstree').jstree(true).settings.core.data.url = 'getItem?sub='+ $('#include-sub').prop('checked');
	...
	function gridview(val){
		...
		jQuery(grid_selector).jqGrid({
			url:'getEmployeeOrg?id='+val+'&sub='+ $('#include-sub').prop('checked'),
            editurl: 'clientArray',
            datatype: "json",
			height: 250,
...
<script type="text/javascript">
	...
	$('#jstree').jstree({		
		'core' : {	
			...
			'data':
				{
			    'url':'getItem?sub='+ $('#include-sub').prop('checked'),
			    'dataType':'json'
			 	}
    ...
    $('#jstree').on("changed.jstree", function (e, data) {				    	
   		...
   		$("#grid-table").jqGrid('setGridParam',{datatype:'json',url: 'getEmployeeOrg?id='+data.selected+'&sub='+ $('#include-sub').prop('checked')}).trigger('reloadGrid');		      			      
    });   
    $('#jstree').on('ready.jstree', function() {
	    $("#jstree").jstree("open_node", $("#94"));     
	});			 						 			
	gridview(94);
	    ...
	    var items = {
	        ...
	        'item2' : {
	            ...
	            'action' : function () { 	  	     
		 			...
		 			bootboxmodal('Edit Section', renameCloneIdsAndNames(html,'Edit'));
		 			$.get('Organization/EditOrg', { id: node.id }, function(data){  			 				
						...
						$("#OrgNameEdit").val(data["data"][0]["OrgName"]);
						...
						$("#ExtentionEdit").attr('value',data["data"][0]["Ext"]);
						$("#OfficialEdit").attr('value',data["data"][0]["Official"]);
	        ...
	        'item3' : {
	            ...
	            'action' : function () { 
					...
					if (r == true) {
						var id= node.id; 
					    $.get('Organization/DelOrg', { id:id }, function(data){  			 				
			 				$('#jstree').jstree(true).settings.core.data.url = 'getItem?sub='+ $('#include-sub').prop('checked');
							$('#jstree').jstree(true).refresh();
			            });
					} 
