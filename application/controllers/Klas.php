...
class Klas extends CI_Controller {
    ...
    public function getEmployeeOrg()
    {
    	...
    	if($this->input->get('_search')== 'true'){
    		...
    		$row = $this->db->query("SELECT EmployeeId,EmployeeName,NameOrganization as Section,NamePositionStructural as Position, NameWorkingStatus as Status from [Fn_EmpClassTree] ('1','2019-01-01','". $recnumEmployee ."',".$sub.") where ". $filter->rules[0]->field ." like '%". $filter->rules[0]->data."%'")->result_array();
    		 	//echo $this->db->last_query();
    	}else{
    		$row = $this->db->query("SELECT EmployeeId,EmployeeName,NameOrganization as Section,NamePositionStructural as Position, NameWorkingStatus as Status from [Fn_EmpClassTree] ('1','2019-01-01','". $recnumEmployee ."',".$sub.")")->result_array();
    	}
	...
	{		
		...
		$sub = $this->input->get('sub')=='false' ? '0':'1'; 
	  	...
	  	$this->output->set_content_type('application/json')->set_output(json_encode($this->admin->getClassParentOrg()));
	}
