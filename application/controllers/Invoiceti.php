...
class Invoiceti extends CI_Controller {
    ...
    {
      if(CheckMenuRole('invoiceti')){
        redirect("errors");
      }
      $data['title'] = 'Invoice Entry Customer';
			$data['main'] = 'invoice/indexti';
			$data['js'] = 'script/invoiceti';
  ...
  {       
      ...
      $data['penagihan_to'] = $this->input->post('penagihan_to',TRUE);
      $data['sp_no'] = $this->input->post('sp_no',TRUE);
      ...
      if($this->input->post('mode') === "edit"){
          ...
          if(!$result){
              ...
              for ($i=1; $i <= $total ; $i++) { 
                ...
                $data['price_chartered'] = 0;
                $data['subtotal'] = str_replace('.', '',  $this->input->post('sub_'.$i,TRUE));
                $data['keterangan'] = $this->input->post('keterangan_'.$i,TRUE);
