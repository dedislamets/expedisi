...
class Role extends CI_Controller {
  ...
  {
      // $sql = "SELECT A.*,B.`group`,C.`menu`,E.`nama_user`,E.`id_user`
      //         FROM tb_group_menu A 
      //         LEFT JOIN tb_group_role B ON A.`id_group`=B.`id`
      //         LEFT JOIN tb_menu C ON C.`id`=A.`id_menu`
      //         LEFT JOIN `tb_group_user` D ON D.`id_group_role`=B.`id` 
      //         LEFT JOIN tb_user E ON E.`id_user`=D.`id_user`
      //         WHERE E.id_user=1";
      $data = [];
      $parent_key = '1';
